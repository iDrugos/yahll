...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('yahll.openPanel', () => openChatPanel(context)), vscode.commands.registerCommand('yahll.runSetup', () => runSetupWizard(context)));
}
function runSetupWizard(context) {
    ...
    const disposable = vscode.window.onDidCloseTerminal(t => {
        if (t.name === 'Yahll Setup') {
            disposable.dispose();
            vscode.window.showInformationMessage('Yahll is ready!', 'Open Chat').then(choice => {
                if (choice === 'Open Chat') {
...
function openChatPanel(context) {
    ...
    panel = vscode.window.createWebviewPanel('yahllChat', 'Yahll', vscode.ViewColumn.Two, {
        enableScripts: true,
...
function startYahllProcess() {
    ...
    let buffer = '';
    yahllProcess.stdout?.on('data', (data) => {
    ...
    yahllProcess.stderr?.on('data', (data) => {
